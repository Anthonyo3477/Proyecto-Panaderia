...
router.get('/', verificarSesion, async (req, res) => {
  try {
    ...
    const [carro] = await db.execute(`
      SELECT c.id, p.nombre, p.clasificacion, p.descripcion, p.precio, c.cantidad,
...
router.post('/agregar', verificarSesion, async (req, res) => {
  try {
    ...
    const [existente] = await db.execute(
      'SELECT * FROM carro WHERE usuario_id = ? AND producto_id = ?',
      [usuario_id, producto_id]
    );

    if (existente.length > 0) {
      await db.execute(
        'UPDATE carro SET cantidad = cantidad + ? WHERE usuario_id = ? AND producto_id = ?',
        [cantidad, usuario_id, producto_id]
      );
    } else {
      await db.execute(
        'INSERT INTO carro (usuario_id, producto_id, cantidad) VALUES (?, ?, ?)',
...
router.post('/eliminar/:id', verificarSesion, async (req, res) => {
  try {
    ...
    await db.execute(
      'DELETE FROM carro WHERE id = ? AND usuario_id = ?',
      [carro_id, usuario_id]
    );
